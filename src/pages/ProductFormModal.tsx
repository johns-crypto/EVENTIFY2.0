...
function ProductFormModal({ business, product, onClose, onSave }: ProductFormModalProps) {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      let productImageUrl = productForm.imageUrl;
      if (productForm.file) {
        productImageUrl = await uploadImageToCloudinary(productForm.file);
      }
      const updatedProduct = { ...productForm, imageUrl: productImageUrl, file: undefined };

      const businessRef = doc(db, 'businesses', business.id);
      const updatedProducts = product
        ? business.products.map((p, i) => (p === product ? updatedProduct : p))
        ...
        <form onSubmit={handleAddProduct} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-12 pr-4 py-2 sm:py-3 rounded-lg bg-gray-700/50 text-gray-200 border ${formErrors.productName ? 'border-red-500' : 'border-gray-600/50'} focus:outline-none focus:ring-2 focus:ring-yellow-400 placeholder-gray-400 transition-all duration-300 text-sm sm:text-base`}
                placeholder="Product Name"
                aria-describedby={formErrors.productName ? 'productName-error' : undefined}
              />
            </div>
            {formErrors.productName && (
              <p id="productName-error" className="mt-1 text-xs text-red-500">
                {formErrors.productName}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className={`w-full p-2 sm:p-3 rounded-lg bg-gray-700/50 text-gray-200 border ${formErrors.productDescription ? 'border-red-500' : 'border-gray-600/50'} focus:outline-none focus:ring-2 focus:ring-yellow-400 placeholder-gray-400 transition-all duration-300 text-sm sm:text-base`}
              rows={3}
              placeholder="Describe the product"
              aria-describedby={formErrors.productDescription ? 'productDescription-error' : undefined}
            />
            {formErrors.productDescription && (
              <p id="productDescription-error" className="mt-1 text-xs text-red-500">
                {formErrors.productDescription}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              className={`w-full px-4 py-2 sm:py-3 rounded-lg bg-gray-700/50 text-gray-200 border ${formErrors.category ? 'border-red-500' : 'border-gray-600/50'} focus:outline-none focus:ring-2 focus:ring-yellow-400 transition-all duration-300 text-sm sm:text-base`}
          ...
          <div>
            ...
            <label className="flex items-center">
              <input
                ...
                className="mr-2 accent-yellow-400 h-4 w-4"
              />
              <span className="text-gray-300 text-sm">Available in stock</span>
            </label>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-3 py-2 text-gray-200 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-yellow-400 file:text-gray-900 hover:file:bg-yellow-300 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
          ...
          <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
            <button
              type="submit"
              className="w-full px-4 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 rounded-full hover:from-yellow-300 hover:to-yellow-400 transition-all font-semibold shadow-sm hover:shadow-md text-sm sm:text-base"
              disabled={imageUploading}
            >
